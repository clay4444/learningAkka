...
import org.clay.ClusterRoledWorker
import org.clay.message.{FilteredRecord, NginxRecord, Registration}
...
class EventInterceptor extends ClusterRoledWorker {
  ...
  def receive = {
    case MemberUp(member) =>
      ...
      register(member, getCollectorPath)  //只找collector的，如果member是interceptor，则返回的path根本不存在。而且只有Interceptor才会注册，collector不会注册
    case state: CurrentClusterState =>
      // 如果加入Akka集群的成员节点是Up状态，并且是collector角色，则调用register向collector进行注册
      state.members.filter(_.status == MemberStatus.Up) foreach(register(_, getCollectorPath))  //和上面重复了 ？
    ...
    case Registration => {  //接收 processor
